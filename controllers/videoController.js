...
// Upload a movie (Only admin can add)
exports.upload = async (req, res) => {
  try {
    ...
    if (!req.user || !req.user.id || !isAdmin(req)) {
      return res.status(403).json({ message: "Forbidden: Only admins can upload movies" });
    }

    const { title, description, writer, genres } = req.body;
    const videoPath = `/uploads/${req.file.filename}`;
    const posterPath = req.body.posterPath || ""; // Ensure posterPath field exists
    ...
    res.status(201).json({ message: "Movie uploaded successfully!", video });
  } catch (error) {
...
exports.getAllVideos = async (req, res) => {
  try {
    const videos = await Video.findAll({
      ...
      attributes: ['id', 'title', 'description', 'writer', 'genres', 'posterPath', 'videoPath', 'uploadedBy', 'createdAt'],
    ...
    const videos = await Video.findAll({
      where: {
        [Op.or]: [
          ...
          { genres: { [Op.iLike]: `%${query}%` } }
        ],
      },
      attributes: ["id", "title", "genres", "writer", "posterPath", "videoPath"],
...
// Fetch remaining movies excluding the currently playing one
exports.getRemainingVideos = async (req, res) => {
  try {
    ...
    const remainingVideos = await Video.findAll({
      ...
      include: [{ model: User, as: "uploader", attributes: ["fullname"] }],
      attributes: ["id", "title", "posterPath", "videoPath"],
      limit: 20, 
    });
